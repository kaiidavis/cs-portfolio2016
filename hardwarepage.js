...
span.innerHTML = "The motherboard is the main component needed for a computer. It is the base of the computer and each motherboard affects the computer's capabilities and the upgrades that it can have. The way the motherboard is laid out is based on its form factor. Form factor is where everything will go and how the shape of the computer case turns out. All motherboards have a chipset, which is a motherboard logic system that has a northbridge and a southbridge. The computer uses the bridges to connect everything together. The BIOS (Basic Input/Output System) chip makes basic functions and does a self test every time it turns on and maintains the basic settings and the system time. There are many slots and ports in a motherboard. There is a PCI (Peripheral Component Interconnect) connector that works with sound, video, video capture cards, and network cards. Second, AGP (Accelerated Graphics Port) is a port for video cards. Third, IDE (Integrated Drive Electronics) is an interface for hard drives. Then there is either a universal serial bus or a FireWire for external peripherals. Finally there are memory slots to increase the amount of temporary data storage needed to make computers process faster.";
}
...
span.innerHTML = "Ram is random access memory which means it can be accessed at any time. Ram is used as memory for programs who need it for only a limited time that the program is running. The amount of ram decides the speed of accessing instructions and data transmitted.";
}
