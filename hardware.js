...
function img3function() {
    span3.innerHTML = "<p class='whitetext aquaborder col-md-4'> Ram is random access memory which means it can be accessed at any time. Ram is used as memory for programs who need it for only a limited time that the program is running. The amount of ram decides the speed of accessing instructions and data transmitted.</p>";
}
...
function img5function() {
    span5.innerHTML = "<p class='whitetext aquaborder col-md-4'> Bus is a circuit that connects each piece of the motherboard. The speed of the these circuits can move data are measured in MHz(Megahertz). First, the back side bus connects the CPU with the external cache. Second, the memory bus connects the northbridge to the memory. Third, the IDE (Integrated Development Environment) bus connects the southbridge to the disk drives. Also, the AGP (Accelerated Graphics Port) bus connects the video card to the CPU and the memory. Finally, the PCI (Peripheral Component Interconnect) bus connects the southbridge to the PCI slots.</p>";
}
...
function img6function() {
    span6.innerHTML = "<p class='whitetext aquaborder col-md-4'> The chipset takes information from different components and then transmits it to the CPU. The chipset has a north and south bridge and each bridge is connected to parts in a computer. The northbridge is connected to the CPU, the memory, and the AGP (Accelerated Graphics Port) and has the memory controller on it. The southbridge is connected to the PCI (Peripheral Component Interconnect) bus, the USB ports, and the IDE (Integrated Development Environment)/SATA (Integrated Drive Electronics) hard disk connections.</p>";
}
